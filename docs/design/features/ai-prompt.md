...
- 行アンカー: range が 1 行なら `#L<行>`、複数行なら `#L<始>-L<終>` (ともに 1 始まり、new 側ファイルの行番号)
- パスの規則: worktree ありのセッションは worktree 内の絶対 path (`@<worktree>/<file>#L..`)、なしはリポジトリ相対 path。いずれも「開いたエージェントが実際に読める」ことを保証する (MUST 3 の worktree と連動する理由)
...
| キーマップ `y` (diff バッファ) | カーソル行 range に含まれるコメントのみ同上 (outdated の既定除外も同じルール。カーソル行のコメントがすべて outdated の場合はコピーせず INFO「outdated のためプロンプトに含めませんでした」) |
