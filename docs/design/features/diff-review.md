...
- **箱幅** = `min(群内の行の最大表示幅 + 罫線と padding 分, opts.max_width)`、下限 20。`M.apply(session, bufnr, path, opts)` の `opts.max_width` (セル数、nil = 上限なし) は呼び出し側 (handlers/session の apply 4 経路) が head 窓から `nvim_win_get_width(win) - getwininfo(win)[1].textoff` で算出する (commentmarks は窓を探さない = 単体 spec で幅を注入できる)。表示幅は `vim.fn.strdisplaywidth` (CJK・tab・`ambiwidth` を Neovim 側の計算で吸収。罫線文字は East Asian Ambiguous なので横罫線の本数も strdisplaywidth で決め、「+定数」で数えない)。内側幅を超える本文行は表示幅の累積で分割する (単語境界は考慮しない。折り返し行にも continuation pad を付ける。各折り返し行は最低 1 文字を消費する = 内側幅が pad 幅以下でも無限ループしない)。**折り返しで増えた行も 10 行の打ち切り対象に数える**。箱幅は apply 時点の窓幅で固定されるため、`WinResized` / `VimResized` (handlers/session) で同じ経路で再 apply する (多重発火は最後の 1 回で足りる)。他窓 (ユーザーが同バッファを開いた別窓) では head 窓基準の幅で描かれ、狭い窓では右辺が切れる (仕様。「既知の制約」)
...
- 位置を解けない outdated (= new 側に該当テキスト無し) は当該 head バッファ **1 行目の virt_lines_above** に同じ箱で集約: 見出し `N outdated (prompt 除外中)` (hl `ReviewCommentOutdated`) を箱 1 行目に置き、以降はコメント間 `├─┤` 区切り (見出しと先頭コメントは区切りを挟まず隣接)。head 窓が存在しないファイル (deleted・binary 告知窓) に紐づく outdated は panel winbar の末尾要素 `⚠N` (「窓装飾 (chrome)」参照) とプロンプト除外 INFO で可視化する
